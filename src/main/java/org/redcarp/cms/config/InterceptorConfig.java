package org.redcarp.cms.config;
...
public class InterceptorConfig implements WebMvcConfigurer {
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		registry.addInterceptor(new ValidateAuthorizationInterceptor()).excludePathPatterns(excludePathList);
	}
	...
	private class ValidateAuthorizationInterceptor implements HandlerInterceptor {
		...
		public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
			// 核验 Token 并获取用户角色
			String USER_ROLE = JwtToken.verifyTokenAndGetUserRole(request.getHeader("Authorization"),
			                                                      request.getHeader("From"),
			                                                      JWT_SECRET_KEY);

			// 返回字符串开头为“【”，表示 “未通过身份验证” ，返回 401 Unauthorized 错误
			if (USER_ROLE.charAt(0) == '【') {
				response.setStatus(401);
				response.setContentType("application/json;charset=UTF-8");
				response.setHeader("Connection", "close");
				String responseError = "{\n" + "    \"timestamp\": \"" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(
