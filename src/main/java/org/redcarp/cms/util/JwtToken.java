...
public class JwtToken {
	...
	public static String createToken(User user, String jwtSecretKey) {
		...
		JwtBuilder builder = Jwts.builder().setIssuer("RedCarp").setAudience(user.getUsername()) // 受众(aud) -> 存放用户名
	...
	public static String verifyTokenAndGetUserRole(String token, String username, String jwtSecretKey) {
		try {
			Claims body = Jwts.parser().setSigningKey(new SecretKeySpec(jwtSecretKey.getBytes(),
			                                                            SIGNATURE_ALGORITHM.getJcaName())).parseClaimsJws(
					token).getBody();
