...
import org.redcarp.cms.exception.CategoryUpdateException;
import org.redcarp.cms.exception.UserRolePermissionException;
import org.redcarp.cms.repository.ArticleCategoryRepository;
import org.redcarp.cms.repository.ArticleRepository;
import org.redcarp.cms.repository.CategoryRepository;
...
public class CategoryServiceImpl implements CategoryService {
	...
	private final Sort ORDER_BY_SEQUENCE = new Sort(Sort.Direction.ASC, "sequence");
		...
		try {
			...
			for (Field field : category.getClass().getDeclaredFields()) {
				...
				if (field.get(category) != null) {
					String methodName = "set" + field.getName().substring(0,
	...
	public List<Category> moveUpNode(String USER_ROLE, String urlAlias) {
		...
		Category frontCategory = categoryRepository.findByParentNodeUrlAliasAndSequence(currentCategory.getParentNodeUrlAlias(),
		                                                                                currentCategory.getSequence() - 1);
	...
	public List<Category> moveDownNode(String USER_ROLE, String urlAlias) {
		...
		Category currentCategory = categoryRepository.findByUrlAlias(urlAlias);
		Category backCategory = categoryRepository.findByParentNodeUrlAliasAndSequence(currentCategory.getParentNodeUrlAlias(),
		                                                                               currentCategory.getSequence() + 1);
	...
	private void handleDeleteNode(Category category) {
		...
		int currentLevelCount = categoryRepository.countByNodeLevelAndParentNodeUrlAlias(category.getNodeLevel(),
		                                                                                 category.getParentNodeUrlAlias());
		for (int i = currentSequence + 1; i < currentLevelCount + 1; i++) {
			Category tempCategory = categoryRepository.findByParentNodeUrlAliasAndSequence(category.getParentNodeUrlAlias(),
			                                                                               i);
