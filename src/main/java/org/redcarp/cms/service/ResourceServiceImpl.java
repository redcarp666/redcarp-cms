...
import org.redcarp.cms.exception.ResourceStorageException;
...
public class ResourceServiceImpl implements ResourceService {
	...
	public Resource upload(MultipartFile file) {
		...
		// 存库，数据库标识
		return resourceRepository.save(new Resource(location.toString().replace('\\', '/'),
		                                            originalFilename,
		                                            storageFilename,
		                                            fileType,
		                                            fileSize,
	...
	public ResourceFilterConditions getFilterConditions() {
		...
		try {
			...
			throw new ResourceStorageException("【资源存储异常】- 资源文件夹 " + uploadDir + " 下日期分类列表获取失败！",
			                                   e);
		}
